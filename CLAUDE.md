...
- After finishing a QA pass, clean up artifacts generated by playwright-cli. That includes everything inside `.playwright-cli/` as well as any screenshots or snapshot YAML files that landed in the repo root (e.g. files matching `*.png`, `*.yaml` with timestamps or descriptive names from the session).
