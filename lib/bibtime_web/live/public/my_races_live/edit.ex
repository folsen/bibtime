defmodule BibtimeWeb.Public.MyRacesLive.Edit do
  ...
  def mount(%{"participant_id" => participant_id}, _session, socket) do
    ...
    participant = Participants.get_participant!(participant_id) |> Bibtime.Repo.preload([:race, :race_category])
    ...
    if participant.user_id != user.id do
      ...
      {:ok,
       assign(socket,
         ...
         page_title: "Edit Registration — #{race.name}"
       )}
    end
  end
    ...
    <div class="max-w-2xl mx-auto px-4 py-10">
      ...
      <div class="mb-8">
        ...
        <p class="text-base-content/50">
          {@race.name}
          <span :if={@participant.bib_number} class="font-mono"> — Bib #{@participant.bib_number}</span>
        </p>
      </div>
      ...
      <div class="rounded-xl bg-base-100 border border-base-300/50 shadow-sm">
        ...
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <.input field={@form[:first_name]} type="text" label="First Name" required disabled={!@editable?} />
            <.input field={@form[:last_name]} type="text" label="Last Name" required disabled={!@editable?} />
          </div>

          <.input field={@form[:email]} type="email" label="Email" required disabled={!@editable?} />
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <.input
              ...
              disabled={!@editable?}
            />
            <.input field={@form[:birth_date]} type="date" label="Birth Date" disabled={!@editable?} />
          </div>

          <.input field={@form[:club]} type="text" label="Club / Team" placeholder="(optional)" disabled={!@editable?} />
